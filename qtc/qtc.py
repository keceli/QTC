...
def run(s):
    ...
    if io.check_dir(parameters['qctemplate']):
        ...
        parameters['qctemplate'] = io.join_path(
            *[parameters['qtcdirectory'], 'templates'])
    ...
    if io.check_file(runfile):
        if over:
            ...
            logging.warning('Skipping calculation since it is already running. Use -O {1}.{2} to overwrite or delete "{0}" file'.format(
                io.get_path(runfile), parameters['mol_index'], calcindex))
    elif ignore and task != 'composite':
        runqc = False
    if task == 'composite':
        runqc = True
    runtime = 0
        ...
        try:
            if qcpackage in available_packages:
                ...
                if io.check_file(geofile, 1):
                    qc.run_qcscript(
                        qcscript, parameters['qctemplate'], geofile, mult)
            elif parameters['qcmethod'] == 'given':
                qc.use_given_hof(parameters)
                io.rm(runfile)
            else:
                logging.error('{0} package not implemented.\nAvailable packages are {1}'.format(
                    qcpackage, available_packages))
            ...
            if parameters['debug']:
                ...
                logging.error(
                    'Error in running quantum chemistry calculations')
    ...
    if parseqc:
        ...
        if parameters['qctask'] == 'composite':
            ...
            if io.check_file(enefile, 0, True):
                ...
                if runthermo:
                    logging.error(
                        'Cannot run thermo, no energy file "{0}".\n'.format(enefile))
                    runthermo = False
                else:
            ...
            if qc.check_output(out):
                try:
                    results = qc.parse_output(
                        out, formula, parameters['writefiles'])
                except Exception as e:
                    if 'opt' in task:
                        parameters['break'] = True
                    if parameters['debug']:
                        ...
                        logging.error('Error in parsing {}'.format(
                            io.get_path(qcoutput)))
                        exc_type, exc_obj, exc_tb = sys.exc_info()
                        fname = os.path.split(
                ...
                if runthermo:
                    logging.error(
                        'Cannot run thermo failed calculation in "{0}"'.format(qcoutput))
                    runthermo = False
                else:
                    logging.error(
                        'Found failed calculation "{0}"'.format(qcoutput))
            if 'xyz' in results and natom > 1:
                ...
                try:
                    ...
                    logging.error(
                        'Exception {}. Final xyz can not be converted to INCHI -- Using original inchi'.format(e))
                    inchifinal = inchi
...
#########################
            ...
            if 'opt' in task:
                parameters['break'] = True
            logging.error(
                'Output file "{0}" not found in {1}.'.format(qcoutput, io.pwd()))
            if runthermo:
    ...
    if 'hindered potential' in parameters['results'] and (task.startswith('tors') or task.startswith('md')):
        tc.get_hindered_potential(
            parameters['results']['hindered potential'], report=parameters['debug'])
    ...
    for key in list(results.keys()):
        ...
        if hasattr(val, '__iter__'):
            if len(list(val)) > 0:
                ...
                if 'freqs' in key:
                    ...
                    logging.info('{:10s} = {}'.format(
                        key, ['{:6.1f}'.format(freq) for freq in floatfreqs]))
            ...
            if val:
                parameters['all results'][slabel][qlabel].update(
                    {key: results[key]})
    ...
    if runthermo:
            ...
            if natom == 1:
                ...
                if x2zinp and natom > 1:
                    try:
                        ...
                        logging.error(
                            'x2z run failed, sym. number is set to 1. Probably a failed xyz')
                else:
                    bonds = {}
        ...
        if hof and parameters['qcmethod'] == 'given':
            ...
            if formula in ['XH2', 'XO2', 'XN2']:  # Remove X to use the definided values
                ...
                hfset = 'Definition'
                logging.info(
                    'Heat of formation of {} is set to 0 by definition.'.format(formula))
            else:
        ...
        if formula in ['XH2', 'XO2', 'XN2']:  # Remove X to use the definided values
            ...
            try:
                hof298, chemkintext, rmgpoly = tc.write_chemkin_polynomial(
                    mol, parameters)
                    ...
                    for uncert in uncertainty:
                        ...
                        if typ[0].lower() in types:
                            if types[typ[0].lower()] in parameters['results']:
                                ...
                                hscale = float(hscale)
                                scales = np.arange(
                                    lscale, hscale, (hscale-lscale) / 9)
                                scales = np.append(
                                    scales, scales[-1] + (hscale-lscale) / 9)
                                for scale in scales:
                                    parameters['scale'] = scale
                                    parameters['scaletype'] = typ
                                    io.mkdir(
                                        'scale{}_{:.3f}'.format(typ, scale))
                                    io.cd('scale{}_{:.3f}'.format(typ, scale))
                                    if not io.check_file('new.groups'):
                                        groupstext = tc.get_new_groups()
                                        io.write_file(groupstext, 'new.groups')
                                    _, tempchemkintext, _ = tc.write_chemkin_polynomial(
                                        mol, parameters)
                                    parameters['all results'][slabel][qlabel]['scalechemkin_{}-{:.3f}'.format(
                                        typ, scale)] = tempchemkintext
                            ...
                            if pfout:
                                ...
                                for l, line in enumerate(pfnew):
                                    ...
                                    T, zeroth, first, second = float(T), float(
                                        zeroth), float(first), float(second)
                                    newzeroth = zeroth + dof * \
                                        np.log(1 + c * T / 1000)
                                    newfirst = first + dof / (1000 / c + T)
                                    newsecond = second + \
                                        dof / (1000 / c + T)**2
                                    pfnew[l] = '{:3.1f}\t{:.4f}\t{:.8f}\t{:.5e}'.format(
                                        T, newzeroth, newfirst, newsecond)
                                pfnew = hdr + '\n' + '\n'.join(pfnew)
                                ...
                                parameters['all results'][slabel][qlabel]['scalechemkin_{}-{:.3f}'.format(
                                    typ, c)] = tempchemkintext
                                parameters['scaletype'] = None
                                io.cd('..')
                            else:
                                logging.info('Cannot scale Q, no pf.dat in {}'.format(
                                    os.path.sep.join(io.pwd().split(os.path.sep)[:-1])))
                        else:
                            logging.info(
                                'Invalid uncertainty analysis type {}'.format(typ))
                        io.cd('..')
                    parameters['scale'] = 0
...
def main(arg_update={}):
    ...
    if parameters['swift']:
        ...
        if mpisize > 1:
            ...
            if (mpirank % qcnproc) == 0:
                ...
                qtcruninfo += 'Using {} cores for a single quantum chemistry calculation'.format(
                    qcnproc)
                beginindex = parameters['first'] + qcrank
                qtcruninfo += 'Begin index shifted to {} for qcrank {} mpirank {}'.format(
                    beginindex, qcrank, mpirank)
            else:
                sys.exit()
        else:
    ...
    if runthermo:
        ...
        for s in mylist:
            ...
            if 'cbh' in ref.lower():
                _, basismolecules, _ = hf.cbh_coefficients(
                    s.split('_')[0], ref, parameters)
            else:
                _, basismolecules, _ = hf.comp_coefficients(
                    formula, basis=parameters['reference'].split(','))
            for basismol in basismolecules:
                smi = qc.get_slabel(basismol)
                molbasis.append(basismol)
                if smi not in mylist:
                    msg = '{0} added to input list for heat of formation calculation of {1}'.format(
                        basismol, s)
                    mylist = [smi] + mylist
                    logging.info(msg)
        ...
        logging.info(
            "QTC: Number of species required for thermo= {0}".format(len(mylist)))
    if parameters['generate']:
        ...
        if io.check_file(sortedfile, 1):
            ...
            logging.info(
                'You can use qtc -b 1 -e 5, to compute species with indices 1,2,3,4,5.')
        else:
            logging.error(
                'Problem in writing sorted SMILES file {}'.format(sortedfile))
        ...
        if runthermo:
            ...
            for mid, s in enumerate(mylist):
                ...
                for i in range(ncalc):
                    if parameters['break']:
                        logging.info(
                            'Skipping next calculations for {}'.format(s))
                        break
                    parameters['calcindex'] = i
                    parameters['qctemplate'] = templatedir
                    logging.info('\n' + 50*'-' + '\n')
    ...
    if parameters['all results']:
        ...
        if runthermo:
            ckin = ''
            out = '{0:5s} {1:30s} {2:>15s} {3:>15s}\t   {4}\n'.format(
                'IDX', 'SMILES', 'DeltaH(0)', 'DeltaH(298)', 'Key')
            out += '{0:5s} {1:30s} {2:>15s} {3:>15s}\t   {4}\n'.format(
                '   ', '      ', '[kj/mol]', '[kj/mol]', '  ')
            for i, s in enumerate(mylist):
                sresults = parameters['all results'][qc.get_slabel(s)]
            ...
            ckinfile = 'chemkin_' + \
                parameters['logfile'] + \
                get_date_time("_%y%m%d_%H%M%S") + '.txt'
            ckinfile = io.get_unique_filename(ckinfile)
            io.write_file(ckin, ckinfile)
            logging.info('Written all chemkin polynomials in {}'.format(
                io.get_path(ckinfile)))
        if parameters['dumpjsonfile']:
            jsonfile = 'qtc_parameters_' + \
                get_date_time("%y%m%d_%H%M%S") + '.json'
            ...
            if jlist:
                prefix = inp.split('.')[0]
                for i in range(ncalc):
                    csvfile = prefix + '_method_' + \
                        str(i) + get_date_time("_%y%m%d_%H%M%S") + '.csv'
                    ...
                    for d in jlist:
                        ...
                        try:
                            ...
                            Cplist = [tc.get_heat_capacity(poly, T) for T in [
                                300, 500, 1000, 1500]]  # cal/mol*K
                ...
                csvfile = prefix + '_rmg_' + \
                    get_date_time("_%y%m%d_%H%M%S") + '.csv'
                csvfile = io.get_unique_filename(csvfile)
                csvtext = '{},{},{},{},{},{},{},{},{},{},{}\n'.format(
                ...
                for d in jlist:
                    ...
                    try:
                        ...
                        value = float(d['Value'])
                        Cplist = [float(
                            cp) / ut.kcal2kj for cp in [d['Cp300'], d['Cp500'], d['Cp1000'], d['Cp1500']]]
                        S298 = float(d['S298']) / ut.kcal2kj
                        H298 = float(d['H298']) / ut.kcal2kj / 1000.
                        csvtext += '{},{},{},{},{},{},{},{},{},{},{}\n'.format(
                            s, name, sensitivity, uncertainty, value, H298, S298, Cplist[0], Cplist[1], Cplist[2], Cplist[3])
                    except:
                        csvtext += '{},{},{},{},{},{},{},{},{},{},{}\n'.format(
                            s, name,        'NA',        'NA',  'NA', 'NA', 'NA',      'NA',      'NA',     'NA',      'NA')
                if csvtext:
                    logging.info('Writing csv file {}'.format(csvfile))
                    io.write_file(csvtext, csvfile)
