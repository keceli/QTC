...
def gaussian_xyz_foresk(lines):
    ...
    if 'Eckart' in lines:
        ...
        lines = lines.split(
            'Coordinates (Angstroms)')[-1].split(' Distance matrix')[0]
        lines = lines.split('\n')[3:-2]
        for i, line in enumerate(lines):
            line = line.split()
            xyz += atoms[i] + '  ' + line[3] + \
...
def gaussian_geo(lines):
    ...
    try:
        if 'Eckart' in lines:
            ...
            for i, line in enumerate(lines):
                line = line.split()
                xyz += atomnum[line[1]] + '  ' + line[2] + \
                    '  ' + line[3] + '  ' + line[4] + '\n'
        else:
            lines = lines.split('Coordinates (Angstroms)')[-1].split(
                ' Distance matrix')[0].split(' Rotation')[0].split('Symm')[0]
            lines = lines.split('\n')[3:-2]
            for i, line in enumerate(lines):
                line = line.split()
                xyz += ' ' + atomnum[line[1]] + '  ' + \
...
def molpro_zmat(lines):
    ...
    if 'OPTG' in lines:
        optzmat = True
        lines = lines.split('END OF GEOMETRY OPTIMIZATION')[
            0].split('Variable')[-1].split('\n')
        lines = lines[3:-3]
        for line in lines:
