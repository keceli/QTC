...
def sort_species_list(slist, printinfo=False, byMass=False):
    ...
    for s in slist:
        ...
        if len(isomers) > 1:
            logging.info('{} isomers found for {} : {}'.format(
                len(isomers), s, isomers))
        for isomer in isomers:
            ...
            tmplist.append([isomer, formula, smult, obmult,
                           nrotor, nelec, natom, nheavy, mass])
    if byMass:
        tmplist = sorted(tmplist, reverse=False,
                         key=lambda x: (x[8], x[4], x[5], x[6]))
    else:
        tmplist = sorted(tmplist, reverse=True,
                         key=lambda x: (x[4], x[5], x[6]))

    sortedlist = [x[0] for x in tmplist]
    if printinfo:
        logging.info('-'*100)
        if byMass:
            logging.info('{:>8s}\t{:30s} {:20s} {:>8s} {:>8s} {:>8s} {:>8s} {:>8s} {:>8s}   {:>8s}'.format(
                'Index', 'SMILES', 'Formula', 'Mult', 'OBMult', 'N_rot', 'N_elec', 'N_atom', 'N_heavy', 'Mass'))
        else:
            logging.info('{:>8s}\t{:30s} {:20s} {:>8s} {:>8s} {:>8s} {:>8s} {:>8s} {:>8s}'.format(
                'Index', 'SMILES', 'Formula', 'Mult', 'OBMult', 'N_rot', 'N_elec', 'N_atom', 'N_heavy'))
        i = 0
        for tmp in tmplist:
            i += 1
            if byMass:
                logging.info('{:8d}\t{:30s} {:20s} {:8d} {:8d} {:8d} {:8d} {:8d} {:8d}   {:6f}'.format(
                    i, ob.get_smiles(tmp[0]), *tmp[1:]))
            else:
                logging.info('{:8d}\t{:30s} {:20s} {:8d} {:8d} {:8d} {:8d} {:8d} {:8d}'.format(
                    i, ob.get_smiles(tmp[0]), *tmp[1:]))
        logging.info('-'*100)
...
def add_species_info(s, parameters):
    ...
    if parameters['natom'] > 1 and io.check_exe(parameters['x2z']):
        ...
        logging.info(
            'x2z not found, using number of rotors computed by open babel')
        parameters['nrotor'] = ob.get_nrotor(mol)
    io.cd(pwd)
    return parameters
...
def get_input(x, template, parameters):
    ...
    if task.startswith('tors') or task.startswith('md'):
        ...
        if parameters['anharmonic'] == True:
            inp = inp.replace(
                'QTC(ANHARMLOC)', parameters['optlevel'] + '/' + parameters['freqlevel'])
        else:
        ...
        if package == 'nwchem':
            ...
            if "QTC(EXTRA)" in inp:
                if nopen == 0:
                    extraline = '{{multi;closed,{0};occ,{1};wf,{2},1,{3};canonical}}'.format(
                        int(nelectron/2), int(nelectron/2), nelectron, nopen)
                elif nopen == 1:
                    extraline = '{{multi;closed,{0};occ,{1};wf,{2},1,{3};canonical}}'.format(
                        int((nelectron-1)/2), int((nelectron+1)/2), nelectron, nopen)
                elif nopen == 2:
                    extraline = '{{multi;closed,{0};occ,{1};wf,{2},1,{3};canonical}}'.format(
                        int((nelectron-2)/2), int((nelectron+2)/2), nelectron, nopen)
                elif nopen == 3:
                    extraline = '{{multi;closed,{0};occ,{1};wf,{2},1,{3};canonical}}'.format(
                        int((nelectron-3)/2), int((nelectron+3)/2), nelectron, nopen)
                elif nopen == 4:
                    extraline = '{{multi;closed,{0};occ,{1};wf,{2},1,{3};canonical}}'.format(
                        int((nelectron-4)/2), int((nelectron+4)/2), nelectron, nopen)
                else:
                    logging.warning(
                        '{} input not implemented for multiplicity {}'.format(method, mult))
...
def update_smiles_list(slist):
    ...
    for s in slist:
       ...
       # if 'He' in s or 'Ne' in s or 'Ar' in s or 'Kr' in s or 'Xe' in s or 'Rn' in s:
        if 'He' in s or 'Ne' in s or 'Ar' in s or 'Kr' in s or 'Xe' in s or 'Rn' in s:
            logging.info(
                'Inert species {0} is removed from the smiles list'.format(s))
        else:
            if '_e' in s:
                s, ene = s.split('_e')
            if '_s' in s:
            ...
            if '_m' in s:
                ...
                mult = ob.get_mult(s)
                logging.debug(
                    "Multiplicity is set by open babel. {} : {}".format(smi, mult))
            canonical = ob.get_smiles(s)
            if canonical.strip() == smi:
                pass
            else:
                logging.debug(
                    'SMILES changed after open babel canonicalization {} --> {}'.format(smi, canonical))
...
def parse_qckeyword(parameters, calcindex=0):
    ...
    if parameters['natom'] == 1:
        ...
        if task.startswith('comp') or task.startswith('ene'):
            ...
            logging.info(
                'Replacing Task = {} with Task = energy, since there is only a single atom'.format(task))
            task = 'energy'
            parameters['task'] = task
    elif parameters['nallrotor'] == 0:
        if task.startswith('torsopt'):
            logging.info(
                'Replacing Task = {} with Task = opt, since there are no torsions.'.format(task))
            task = 'opt'
        elif task.startswith('torsscan'):
            logging.info(
                'Replacing Task = {} with Task = optfreq, since there are no torsions.'.format(task))
            task = 'optfreq'
        ...
        if task.startswith('md'):
            logging.info(
                'Replacing Task = {} with Task = torsscan, since there is only one torsion.'.format(task))
            task = 'torsscan'
    if task.startswith('ext') or task.startswith('cbs') or task.startswith('comp'):
        ...
        if task == 'torsopt':
            qcdirectory = io.fix_path(io.join_path(
                *[task, method, basis, package]))
            parameters['optdir'] = qcdirectory
        elif task == 'optfreq':
            qcdirectory = io.fix_path(io.join_path(
                *['opt', method, basis, package, 'freq', method, basis, package]))
            parameters['optdir'] = io.fix_path(
                io.join_path(*['opt', method, basis, package]))
            parameters['freqdir'] = qcdirectory
        elif task == 'optanharm':
            qcdirectory = io.fix_path(io.join_path(
                *['opt', method, basis, package, 'anharm', method, basis, package]))
            parameters['optdir'] = io.fix_path(
                io.join_path(*['opt', method, basis, package]))
            parameters['freqdir'] = qcdirectory
        elif 'opt' in task:
            qcdirectory = io.fix_path(io.join_path(
                *['opt', method, basis, package]))
            parameters['optdir'] = qcdirectory
        elif task == 'torsscan' or task == 'md':
            qcdirectory = io.fix_path(io.join_path(
                *[optdir, task, method, basis, package]))
            parameters['freqdir'] = qcdirectory
...
def parse_output(s, formula, write=False):
    if isinstance(s, list):
        ...
        logging.info(
            "First parameter in parse_output should be a string or a list of strings")
    ...
    if package == 'nwchem':
        ...
        # will figureout source of this later
            ...
            if io.check_dir('me_files', 1):
                try:
                    ...
                    try:
                        ...
                        logging.error(
                            'parse_output: torsscan optimization did not converge')
                except:
                    logging.error('parse_output: Cannot parse me_files {}'.format(
                        io.get_path('me_files')))
        ...
        if write:
            ...
            if sum(afreqs) > 0:
                if any(freq < 0 for freq in afreqs):
                    logging.error('Imaginary frequency detected: {}'.format(
                        ['{:6.1f}'.format(freq) for freq in afreqs]))
                fname = formula + '.anhrm'
...
def get_output_data(out, package=None):
    ...
    if package == 'nwchem':
        ...
        # will figureout source of this later
        data['method'] = method.replace('\(', '(').replace('\)', ')')
...
def run(s, parameters, mult=None, trial=0):
        ...
        if overwrite:
            ...
            if task.startswith('tors'):
                ...
                if check_output(out):
                    ...
                    if recover:
                        logging.info(
                            'Renaming failed output and trying to recover')
    ...
    if task.startswith('tors'):
        ...
        if trial > 0:
            templatename = '{0}_{1}_{2}_template.txt'.format(
                task, package, trial)
        else:
    ...
    if io.check_file(templatefile) and runqc:
        ...
        if runqc:
            logging.warning(
                'Set runqc False. Cannot locate template file {}'.format(templatefile))
            runqc = False
        recover = False
    if runqc:
        ...
        if io.check_file(inpfile, timeout=1):
            if package in ['nwchem', 'torsscan', 'torsopt', 'md']:
                ...
                if package == 'nwchem':
                    io.mkdir('tmp_nwchem')
                logging.info(
                    'Running quantum chemistry calculation with {}'.format(command))
                ...
                if len(inppath) > 255:
                    logging.info(
                        'Creating {} since path length > 255 (molpro problem)'.format(tmpdir))
                    io.mkdir(tmpdir)
                    io.symlink(tmpdir, 'tmp')
                    io.cp(inpfile, tmpdir)
                    io.cd(tmpdir)
                command = parameters['qcexe'] + \
                    ' ' + inpfile + ' -o ' + outfile
                logging.info(
                    'Running quantum chemistry calculation with {}'.format(command))
                msg += io.execute(command,
                                  stdoutfile='stdouterr.txt', merge=True)
                if len(inppath) > 255:
            ...
            if io.check_file(outfile, timeout=1):
                ...
                if not check_output(out):
                    ...
                    if recover:
                        logging.info(
                            'Attempting to recover, trial {}'.format(trial+1))
...
def run_composite(parameters):
    ...
    for i in range(0, calcindex):
        try:
            ...
            e[i] = 0.
            logging.error('Energy not found for {0} {1}. Exception: {2}'.format(
                slabel, qlabel, err))
    ...
    if energy:
        ...
        energy = 0.
        logging.error(
            'Problem in given composite formula: {}'.format(compositeformula))
    return energy
...
def run_qcscript(qcscriptpath, inputpath, geopath, multiplicity):
    ...
    process = Popen(['perl', qcscriptpath, inputpath, geopath,
                    str(multiplicity)], stdout=PIPE, stderr=PIPE)
    ...
    for line in lines:
        if line.startswith(key):
            try:
                ...
                logging.error(
                    'Failed in parsing sym. number in x2z output {}'.format(out))
    return sym
...
def get_x2z_zmat(out):
    ...
    if out:
        ...
        if (len(measure) % 2 != 0):
            measure = measure[:-1]
        # Puts measurements into two columns
...
def get_x2z_info(out):
    ...
    for line in lines:
        if 'molecule is' in line:
            ...
            line = line.replace('molecular structure:', '').replace(
                '(', ' ').replace(')', ' ')
...
def get_list_info(slist, sep=' , '):
    info = ''
    header = 'Index' + sep + 'Given' + sep + \
        'Can.SMILES' + sep + 'XYZ_SMILES' + sep
    for i, s in enumerate(slist):
...
def get_package(templatename):
    ...
    if 'cbs' in templatename or suffix == 'py':
        p = 'extrapolation'
...
def get_method(s):
    """
    """
    methods = ['b3lyp', 'ccsdt(q)', 'ccsdt', 'ccsd(t)',
               'ccsd', 'mp2', 'mp3', 'pm3', 'pm6', 'pm7']
    method = ''
    for m in methods:
        if m in s.lower:
            method = m
...
def get_basis(s):
    """
    """
    opts = ['aug-cc-pvdz', 'aug-cc-pvtz', 'aug-cc-pvqz',
            'cc-pvdz', 'cc-pvtz', 'cc-pvqz', 'sto-3g', '6-31g']
    basis = ''
    for m in opts:
        if m in s.lower():
            basis = m
    ...
    try:
        while line.strip():
            ...
            for irow in range(icol, nfreq):
                ...
                ncol = len(cols) - 1
                xmat[irow, icol:icol +
                     ncol] = [float(num.replace('D', 'E')) for num in cols[1:]]
            iline += 1
            line = lines[iline]
        xmat = xmat.tolist()
    except:
        logging.warning(
            'Ignoring anharmonicities -- unexpected length of xmat')
        xmat = []
    return xmat
...
def get_nwchem_xyz(inp, filename=False):
 ...
 ---- ---------------- ---------- -------------- -------------- --------------
    ...
    for line in lines[n+2:]:
        ...
        if len(items) == 6:
            geolines += '{0}    {1}     {2}     {3}\n'.format(
                items[1], items[3], items[4], items[5])
            natom += 1
        else:
            break
        ...
        if keyword in line:
            if len(line.split()) == 3:
                try:
                    ...
                    logging.error(
                        'Parser error for get_nwchem_nbasis  in line {}'.format(line))
    return nbasis
        ...
        if line.islower():  # Returns false if line has no letters
            ...
            for item in items:
                try:
                    ...
                    logging.error(
                        'Non-numeric string in frequency lines of mess input: {0}'.format(item))
        else:
            pass
    return sorted(freqs)
